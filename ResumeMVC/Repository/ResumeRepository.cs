...
namespace ResumeMVC.Repository
{
    public class ResumeRepository : IResumeRepository
    {
        ...
        public bool EditDetails(Person person, int? id)
        {
            try
            {
                int nbRecords = 0;
                ...
                if (person != null)
                {
                    Person BasiDetails = _dbContext.People
                                        .Where(x => x.IDPers == id)
                                        .SingleOrDefault();
                    BasiDetails.FirstName = person.FirstName;
                    BasiDetails.LastName = person.LastName;
                    BasiDetails.DateOfBirth = person.DateOfBirth;
                    BasiDetails.Email = person.Email;
                    BasiDetails.Tel = person.Tel;
                    _dbContext.Entry(BasiDetails).State = EntityState.Modified;
                    nbRecords = _dbContext.SaveChanges();
                }
                return nbRecords > 0 ? true : false;
            }
        ...
        {
            Person getall =new Person();
            return  _dbContext.People.Find();
        }
        ...
        public int GetEmaiPerson(string Email)
        {
            int emailcount = _dbContext.People.Where(p => p.Email == Email).Count();
            return emailcount;
        }
